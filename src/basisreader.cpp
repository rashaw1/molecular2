...
std::vector<GaussianShell> BasisReader::read_basis(std::string atom, double* pos, 
...
ECP BasisReader::readECP(int q, ECPBasis& ecpset, double* center) {
	...
	if (ecp_it != ecp_list.end()) {
		...
		std::string filename = 	"./basislibrary/bases/build/ecp" + std::to_string(value) + ".xml"; 
		...
		for (pugi::xml_node shell = atom_node.child("Shell"); shell; shell = shell.next_sibling("Shell")) {
			...
			for (pugi::xml_node nxc = shell.child("nxc"); nxc; nxc = nxc.next_sibling("xc")) {
