...
void G002(ECP& U, GaussianShell& shellA, GaussianShell& shellB, ShellPairData& data, ThreeIndex<double>& values) {
	...
	double base_values[3]; 
	double zA, zB, zC, dA, dB, dC; 
	double p, P1, P2, P1_2, P2_2, oP1, oP2, X1, X2, aAbB, o_root_p, Kab, o2bB; 
	double radial_value = 0.0;
	for (int c = 0; c < npC; c++) {
		GaussianECP& g = U.getGaussian(c);
		...
		if (g.l == 2) {
			...
			for (int a = 0; a < npA; a++) {
				...
				for (int b = 0; b < npB; b++) {
					...
					P2_2 = P2 * P2;
					aAbB = zA*A2 + zB*B2; 
					Kab = 1.0 / (16.0 * zA * A * zB * B);
					X1 = exp(p*P1_2 - aAbB) * Kab;
					X2 = exp(p*P2_2 - aAbB) * Kab; 
					
					compute_base_integrals(2, 4, p, o_root_p, P1, P2, P1_2, P2_2, X1, X2, oP1, oP2, base_values); 
					
					o2bB = 0.5 / (zB * B); 
					double temp = p*p*base_values[2] - (2.0*p*zB*B + 3.0*p*p*o2bB)*base_values[1] + (zB*B*zB*B + p + 3.0*p*p*o2bB*o2bB)*base_values[0];
					temp /= (zA * A * zA * A); 
	...
	radial_value *= ROOT_PI * M_PI * M_PI;
	std::cout << radial_value << std::endl;
